...
const AdminLayout: React.FC<AdminLayoutProps> = ({ children }) => {
  ...
  return (
    <div className="h-screen flex overflow-hidden animate-fade-in admin-layout-bg">
      ...
      {/* Sidebar */}
      <aside className={`fixed inset-y-0 left-0 z-50 transition-all duration-500 ease-in-out flex flex-col admin-sidebar-bg border-r shrink-0 ${isSidebarOpen ? 'w-72 translate-x-0' : 'w-0 -translate-x-full lg:w-24 lg:translate-x-0'
        }`}>
          ...
          {menuItems.map((item) => {
            ...
            return (
              <Link
                key={item.label}
                to={item.path}
                className={`flex items-center gap-4 p-4 rounded-2xl transition-all relative group ${isActive
              ...
              >
                ...
                {item.badge && isSidebarOpen && (
                  <span className={`ml-auto px-2 py-0.5 rounded-lg text-[10px] font-black ${isActive ? 'bg-white text-indigo-600' : 'bg-red-500 text-white'
                    }`}>
                    {item.badge}
                  </span>
                )}
